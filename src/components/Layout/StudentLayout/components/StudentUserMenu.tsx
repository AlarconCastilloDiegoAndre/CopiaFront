...
export const StudentUserMenu = () => {
  ...
  const { data: student } = useAuthUser() as { data: Student | null };
  ...
  return (
    <div className="flex-shrink-0 p-4 border-t border-gray-200">
      <Popover className="relative">
        ...
        >
          <div
            className="w-8 h-8 bg-main-color rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
            {student?.name?.[0] || 'E'}
          </div>
          <div className="flex-1 min-w-0 text-left">
            ...
            <p className="text-xs text-gray-500 truncate">{student.career.name}</p>
          </div>
        </PopoverButton>
